         ...
         $user = \App\User::where('role', '=','Chemist')->where('mobile', '=', $request->mobile)->where('status', '=','verify')->first();
          ...
          $orders = \App\Order::where('user_id','=',$user->id)->get(); 
           foreach($orders as $order){              
              $order_products= \App\OrderProduct::where('order_id','=','NSRID-'.$order->id)->get();
              foreach($order_products as $order_product){ 
                    $product = \App\Product::whereIn('id', [$order_product->product_id])->where('brand_name', 'LIKE', '%' . $request->input('name') . '%')->orWhere('generic_name','LIKE', '%' . $request->input('name') . '%')->first();  
                  if($product){
