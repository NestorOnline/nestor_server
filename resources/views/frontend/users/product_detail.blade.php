...
?>
                                            ...
                                            @else
                                            <input type="number" class="form-control" id="pincode_value"
                                                placeholder="Enter delivery pincode" value="110001" maxlength="6"
                                                aria-label="Username" aria-describedby="">
                                            @endif
                                            <span class="input-group-text b-unset" id="basic-addon1"><a class="font-14"
                                                    href="javascript:void(0);" onclick="calc()">check</a></span>
                                            @if($pincode)
                                            <?php
...
?>
 ...
 if($single_product){
    ...
    $brought_also_products = \App\Product::where('go_live',1)->whereIn('product_code',$brought_also_product_lists->map(function($brought_also){
...
?>
  ...
  text-overflow: ellipsis;height: 25px">Packing: {{$package->Packing_Description}}
                                @if($prod->Prescription_Required=='1')
...
<!-- <span class="save">(save upto {{$prod->sales_schame_customer->SalesScheme_Name}})</span> -->
                        ...
                        <div class="dia-bottom plus_minus_button{{$prod->id}}">
                            @if($prod->get_addtocart_item)
                            @if($prod->get_addtocart_item->doctor_description_id)
            
             <div class="qty-div">
                                    <span class="qty_minus" rel="tooltip" data-original-title="Disable for prescribed Medicine" data-toggle="tooltip">-</span>
                                    <input type="number" required="" id="Qty" name="Qty" class="qty_1" min="1"
                                        value="{{$prod->get_addtocart_item->Qty}}">
                                    <span class="qty_plus" rel="tooltip" data-original-title="Disable for prescribed Medicine" data-toggle="tooltip">+</span>
                                </div>
             @else
             <div class="qty-div">
                                    <span class="qty_minus"
                                        onclick="add_cart_from_search({{$prod->id}},{{$purchase_price}},-1,{{$prod->get_addtocart_item->Qty}})">-</span>
                                    <input type="number" required="" id="Qty" name="Qty" class="qty_1" min="1"
                                        value="{{$prod->get_addtocart_item->Qty}}">
                                    <span class="qty_plus"
                                        onclick="add_cart_from_search({{$prod->id}},{{$purchase_price}},1,{{$prod->get_addtocart_item->Qty}})">+</span>
...
?>
                        <p class="m-0">
                            <b>
                                Price: <em
                                    class="fa fa-inr"></em>{{number_format($purchase_price, 2, '.', '')}}
                            </b>
                        </p>
                        @else
                        Coming Soon
