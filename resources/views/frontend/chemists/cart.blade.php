                    ...
                    <div class="product-details" style="height: 100px">
                        <div class="steps-from">
                            <ul class="steps-ul list-unstyled">
                                ...
                                <li class="li2">Checkout<span> </span></li>
...
foreach ($add_cart_datas as $add_cart_data) {
    $add_cart_data_total = $add_cart_data_total + $add_cart_data->amount * $add_cart_data->Qty + $add_cart_data->amount * $add_cart_data->Qty * 12 / 100 + 0;
}
?>
                ...
                <div class="col-md-12">
                    <div class="py-3">
                        <h3>Cart Items (<span id="cart_items">{{$add_to_cards_qty_sum}}</span>)
                            @if($order_setting->MinimumOrderValueForChemist > $add_cart_data_total)
                            <span id="minimum_order_value" style="margin-left: 200px;color: #eb3b65">Minimum Order Value
                                is Rs. {{$order_setting->MinimumOrderValueForChemist}}/-</span>
                            @else
                            <span id="minimum_order_value"
                                style="margin-left: 200px;color: #eb3b65;display: none">Minimum Order Value is Rs.
                                {{$order_setting->MinimumOrderValueForChemist}}/-</span>
                            @endif
                        </h3>
                    </div>
...
?>
                                                ...
                                                <div class="row m-0">
                                                    ...
                                                    <div class="item-prices text-right">
                                                        <div class="discount-val"><span id="row_itmdiscprice_922086"
                                                                style="color: black"><b>Rs.
                                                                    {{number_format($subtotal, 2, '.', ',')}}</b></span>
                                                            <br>
                                                            @if($product_cart->mrp_price)
                                                            <span class="final_price"
                                                                style="color: black; display: none">
                                                                <b>Rs.
                                                                    {{number_format($mrp_subtotal, 2, '.', ',')}}</b><span>
                                                                    @endif
                                                        </div>
                                                    </div>
  ...
  text-overflow: ellipsis;">Packing:
                                                            ...
                                                            @if($product_cart->mrp_price)
                                                             <p class="form m-0"><b style="color: black">MRP: Rs.
                                                                {{$product_cart->mrp_price->Price}}</b>
                                                            <span class="final_price" style="color: black"><b>| Purchase
                                                                    Price : Rs.
                                                                    {{number_format($add_cart_data->amount, 2, '.', ',')}}/-</b><span>
                                                                    @endif
                                                        </p>
                                                        <p class="form m-0"><span class="price" style="color: #eb3b65">
                                                                @if($product_cart->chemist_price &&
                                                                $product_cart->mrp_price)
                                                                <?php
$total1 = $product_cart->chemist_price->Price + $product_cart->chemist_price->Price * $product_cart->chemist_price->GST / 100;
$margin = 0.00;
$margin = ($product_cart->mrp_price->Price - $total1) * 100 / $total1;
?>
                                                                <strong> Margin On MRP
                                                                    {{number_format($margin, 2, '.', ',')}}% </strong>
...
?>
                                                        ...
                                                        <div>
                                                            <div class="form-group align-select m-0">
                                                                ...
                                                                <input type="hidden"
                                                                    ...
                                                                    value="{{number_format($product_cart->chemist_price->GST, 2, '.', ',')}}">
                                                                @endif
                                                                @if($product_cart->mrp_price)
                                                                <input type="hidden"
                                                                    id="mrp_product_price{{$add_cart_data->product_id}}"
                                                                    value="{{number_format($product_cart->mrp_price->Price, 2, '.', ',')}}">
                                                                @endif
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <div class="deliveryby row m-0 mt-2">
                                                    <div class="date deldate col pl-0">
                                                        <div class="deliveryby" style="display: none"> Dispatched in 2
                                                            to 3 days </div>
                                                    </div>
...
?>
                    ...
                    <div>
                        <div class="totalamt-col">
                            ...
                            <div class="allcalculation">
                                ...
                                <div class="subtoal"><label _ngcontent-tlt-c6="">Taxable Amount</label><span>Rs.<span
                                            id="cart_total_taxable_amount">{{number_format($total, 2, '.', ',')}}</span></span>
                                </div>
                                @if($check_amount > $order_setting->MinimumOrderValueForChemist&&$wallet > 0)
                                <div class="subtoal"><label _ngcontent-tlt-c6="">Reward Points Available</label><span>
                                        <span>{{number_format(\Auth::user()->wallet, 2, '.', ',')}}</span></span>
                                </div>
                                <div class="subtoal"><label _ngcontent-tlt-c6="">Redeem Reward Points
                                    </label><span>
                                        <input class="form-control text-right" type="number" id="user_wallet"
                                            max="{{$wallet}}" min="1" max="500" maxlength="3" style="width: 70px"
                                            value="{{$wallet}}" readonly />
                                        <!-- <button class="btn btn-sm btn-info" onclick="wallet_apply();">Apply</button> -->
                                    </span>
                                </div>

                                @elseif($check_amount > $order_setting->MinimumOrderValueForChemist && $wallet <= 0 && $reward_ledger)
                                <div class="subtoal"><label _ngcontent-tlt-c6="">Reward Points Available</label><span>
                                        <span>{{$reward_ledger->Balance}}</span></span>
                                </div>
                                <div class="subtoal"><label _ngcontent-tlt-c6="">Redeem Reward Points
                                    </label><span>
                                        <input class="form-control text-right" type="number" id="user_wallet" min="1"
                                            max="{{$reward_ledger->Balance}}">
                                        <!-- <button class="btn btn-sm btn-info" onclick="wallet_apply();">Apply</button> -->
                                    </span>
                                </div>
                                @else
                                <span id="user_wallet" style="display: none">{{number_format(0.00, 2, '.',',')}}</span>
                                @endif
                                <div class="shipping-charges"><label _ngcontent-tlt-c6="">GST
                            ...
                            <div class="allcalculation" style="margin-top: -30px">
                                <div class="subtoal"><label _ngcontent-tlt-c6="">Total Margin on MRP</label><span>
                                        Rs.<span
                                            id="tota_margin_on_mrp">{{number_format($tota_margin_on_mrp, 2, '.', ',')}}</span></span>
                                </div>

                                @if($check_amount > $order_setting->MinimumOrderValueForChemist)
                                <div class="p-3" id="payment_button">
                                    <a href="{{route('frontend.checkout')}}" class="add-to-cart">Continue for
                                        payment</a>
                                </div>
                                @else
                                <div class="p-3" id="payment_button" style="display: none">
                                    <a href="{{route('frontend.checkout')}}" class="add-to-cart">Continue for
                                        payment</a>
                                </div>
                                @endif
                ...
                <div>
                    ...
                    <span class="book-now"><a href="{{route('home')}}" class="btn btn-sm btn-block">Back To Home
                            Page</a>
                    </span>
                </div>
                @endif
    ...
    function remove{{$add_cart_data->product_id}}_function() {
        ...
        $.ajax({
            url: "/add_to_carts/remove_cart_chemist",
            data: {
            ...
            success: function(data) {
                ...
                var user_wallet = parseFloat(user_wallet.replace(',', '').replace(' ', ''));
                ...
                var OrderSetting = '<?php echo $order_setting->MinimumOrderValueForChemist; ?>';
                ...
                var total_margin_on_mrp = mrp_total - cart_total_charge;

                document.getElementById("tota_margin_on_mrp").innerHTML = thousands_separators((
                    total_margin_on_mrp).toFixed(
                    2));
                ...
                document.getElementById("cart_sub_total").innerHTML = thousands_separators((
                        cart_sub_total)
                    .toFixed(2));
                document.getElementById("cart_total_taxable_amount").innerHTML = thousands_separators((
                        cart_sub_total)
                    .toFixed(2));
    ...
    function quantity{{$add_cart_data->product_id}}_change() {
        ...
        var gst = $("#gst{{$add_cart_data->product_id}}").val();
        $.ajax({
            url: "/add_to_carts/change_qty_chemist",
            data: {
                Qty: Qty,
                key_value: key_value,
            ...
            success: function(data) {
                ...
                document.getElementById("cart_total_taxable_amount").innerHTML = thousands_separators((
                        cart_sub_total)
                    .toFixed(2));
                document.getElementById("grand_total_amount").innerHTML = (grand_total_amount).toFixed(
                    2);
                document.getElementById("total_amount_update").innerHTML = (grand_total_amount).toFixed(
                    2);
    ...
    function quantity{{$add_cart_data->product_id}}_change_minus() {
        ...
        var Qty = Number.parseInt(Qty)-1;
            $.ajax({
                ...
                success: function(data) {
                    if(data.status){
                          $(".delete_mem{{$add_cart_data->product_id}}").html(data.data);
                    var cart_total_charge = document.getElementById("cart_total_charge").innerHTML;
                    var cart_total_charge = parseFloat(cart_total_charge.replace(',', '').replace(' ',
                        ''));
                    var user_wallet = document.getElementById("user_wallet").innerHTML;
                    ...
                    document.getElementById("cart_total_taxable_amount").innerHTML =
    ...
    function wallet_apply() {
        ...
        var grand_total_amount = document.getElementById("grand_total_amount").innerHTML;
        var grand_total_amount = parseFloat(grand_total_amount.replace(',', '').replace(' ', ''));
        var updated_grand_total_amount = grand_total_amount - user_wallet;
        document.getElementById("grand_total_amount").innerHTML = thousands_separators((updated_grand_total_amount)
            .toFixed(
                2));
    }
    </script>
    ...
    $("#user_wallet").on('input', function() {
        ...
        document.getElementById("payment_button").innerHTML =
            "<a href='{{$site_route}}/frontend/checkout?reward=" + user_wallet +
