...
" . $card_product->package->Packing_Description . "</span>";
                ...
                if ($sales_scheme) {
                    ...
                    if($sales_scheme->SchemeOn_Code==1){
                    $dividend = $prod->Qty;
  ...
  text-overflow: ellipsis;'>" . $card_product->package->Packing_Description . "</span>";
    ...
    public function add_cart(Request $request)
    {
        ...
        foreach ($product1 as $prod) {
            $totalqty_data = $totalqty_data + $prod->Qty;
    ...
    public function remove_cart_user(Request $request)
    {
        ...
        $total_qty_in_cart = \App\Addtocard::where('user_id',\Auth::user()->id)->sum('Qty');
        $add_to_cards = \App\Addtocard::where('user_id', '=', \Auth::user()->id)->get();
        $bcd = "";
        $grand_total = 0;
        foreach ($add_to_cards as $key => $product) {
        ...
        $abcd = "<a href='" . $host . "/frontend/cart'>
                            <div class='text'>
                        <span class='counter-number' id='counter-number'>" . $total_qty_in_cart . "</span>
                    </div>
                    </a>
                    <div class='cart-dropdown'>
                    <div>" . $bcd . "<div class='pl-3 pr-3'>
                ...
                foreach ($add_to_cards as $add_to_card) {
                    ...
                    if ($product->stock_by_office($Global_Office_Code)) {
                        if ($product->stock_by_office($Global_Office_Code)->QtyForNewOrder > 0) {
                            $add_to_card->stock_in = $product->stock_by_office($Global_Office_Code)->QtyForNewOrder;
                        } else {
                            $add_to_card->stock_in = 0.00;
                        }
                    } else {
                        $add_to_card->stock_in = 0.00;
                    }
                    ...
                    if ($chemist_product_price) {
                        ...
                        $total_chemist_amount = $chemist_product_price->Price * $add_to_card->Qty;
                    ...
                    if ($product->mrp_price && $chemist_product_price) {
                        $total1 = $chemist_product_price->Price + $chemist_product_price->Price * $chemist_product_price->GST / 100;
                        $margin = 0.00;
        ...
        if ($user) {
            ...
            if (count($add_to_cards)) {
                ...
                foreach ($add_to_cards as $add_to_card) {
                    ...
                    $customer_product_price = \App\Productprice::where('Product_Code', '=', $product->product_code)->where('ProductPriceType_Code', '=', '9')->first();
                    $mrp_product_price = \App\Productprice::where('Product_Code', '=', $product->product_code)->where('ProductPriceType_Code', '=', '10')->first();
                    ...
                    $sub_total = $customer_product_price->Price * $add_to_card->Qty ;
                  
                    $total = $total + $sub_total;
                    $add_to_card->offer = "10 % Off";

                    if ($customer_product_price) {
                        $add_to_card->actual_amount = number_format($customer_product_price->Price, 2, '.', ',');
                    } else {
                        $add_to_card->actual_amount = null;
                    }

                    if ($customer_product_price) {
                        $add_to_card->chemist_amount = number_format($customer_product_price->Price, 2, '.', ',');
                        $gst_amount = ($customer_product_price->GST * $sub_total) / 100;
                        $gst_total = $gst_total + $gst_amount;
                        $add_to_card->gst = $customer_product_price->GST . " %";
                ...
                $amount_with_tax = $total + $gst_total;
                $grand_total = $total + $gst_total;
                $data1['type'] = 'Products';
                $data1['data'] = $add_to_cards;
                $data[] = $data1;
                ...
                $data2['message'] = 'View Price Detail';
                $data2['type'] = 'Total Amount';
                $data2['data'] = array('taxable_amount' => number_format($total, 2, '.', ','), 'tax_amount' => number_format($gst_total, 2, '.', ','), 'amount_with_tax' => number_format($amount_with_tax, 2, '.', ','),'delivery_charge'=>$delivery_charge, 'grand_total' => number_format($grand_total, 2, '.', ','));
                $data[] = $data2;
    ...
    public function customer_re_order($id)
    {
...
if ($product->sales_schame_customer) {
    ...
    $purchase_price =$current_price; 
                            ...
                            $addtocard_product->amount = $purchase_price;
                            if($product->Prescription_Required==1){
                                $addtocard_product->doctor_description_id = $order->doctorappointment_id;
                            }
                            $addtocard_product->save();
                        }
                    }
        ...
        if ($order) {
            foreach ($order->orderproducts as $orderproduct) {
                ...
                if ($addtocard_product) {
                    ...
                    if(\Auth::user()->role=='User'){
                        if ($product->customer_price) {
                            $addtocard_product->amount = $product->customer_price->Price +  $product->customer_price->Price* $product->customer_price->GST/100;
                            if($product->Prescription_Required==1){
                                $addtocard_product->doctor_description_id = $order->doctorappointment_id;
                            }
                            $addtocard_product->save();
