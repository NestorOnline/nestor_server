...
class SearchController extends Controller
{
    ...
    {
        // $products = \App\Product::where('products.go_live',1)->where('category_id','=',$request->category_id)->get();
        ...
        if (\Auth::user()) {
            if (\Auth::user()->role == 'Chemist') {
                if ($request->category_id) {
                    ...
                    $products = DB::table('products')->where('products.go_live',1)->where('group_id', '=', $group_id)
                    ...
                    $product_maxs = DB::table('products')->where('products.go_live',1)->where('group_id', '=', $group_id)
                        ->join('productprices', function ($join) use ($min_price, $max_price) {
                            $join->on('products.product_code', '=', 'productprices.product_code')
                                ->where('productprices.ProductPriceType_Code', '=', 7)
                                ->where('productprices.Price', '>', $min_price)
    ...
    public function groupcategory_product_by_price(Request $request)
    {
        ...
        if (\Auth::user()) {
            if (\Auth::user()->role == 'Chemist') {
                $products = DB::table('products')->where('products.go_live',1)->where('groupcategory_id', '=', $groupcategory_id)
                    ->join('productprices', function ($join) use ($min_price, $max_price) {
                        $join->on('products.product_code', '=', 'productprices.product_code')
                            ->where('productprices.ProductPriceType_Code', '=', 7)
                            ->where('productprices.Price', '>', $min_price)
                ...
                $product_maxs = DB::table('products')->where('products.go_live',1)->where('groupcategory_id', '=', $groupcategory_id)
                    ->join('productprices', function ($join) use ($min_price, $max_price) {
                        $join->on('products.product_code', '=', 'productprices.product_code')
                            ->where('productprices.ProductPriceType_Code', '=', 7)
                            ->where('productprices.Price', '>', $min_price)
    ...
    public function customer_get_search_data_after_filter_App(Request $request)
    {
        ...
        if (count($products)) {
            foreach ($products as $product) {
                ...
                if ($product->customer_mrp_price) {
                    ...
                    $product->customer_mrp_price = null;
                }
                ...
                $package = \App\Package::find($product->package_id);
                if ($package) {
                    $product->package = $package->Packing_Description;
                }
    ...
    public function all(Request $request)
    {
        ...
        $products = \App\Product::where('products.go_live',1)->where('brand_name', 'LIKE', $request->search_names . '%')->orWhere('generic_name', 'LIKE', $request->search_names . '%')->get();
        if (strlen($request->search_names) >= 1) {
            $product1s = \App\Product::where('products.go_live',1)->where('brand_name', 'LIKE', '%' . $request->search_names . '%')->where('ProductBrand_Code', 2)->get();
            if ($product1s) {
                $products = $products->merge($product1s);
            }
            $product2s = \App\Product::where('products.go_live',1)->where('brand_name', 'LIKE', '%' . $request->search_names . '%')->where('ProductBrand_Code', 3)->get();
            if ($product1s) {
        ...
        if (strlen($request->search_names) >= 3) {
            ...
            if (count($ProductHashTags)) {
                ...
                $product1s = \App\Product::where('products.go_live',1)->whereIn('product_code', $ProductHashTagDetails->map(function ($ProductHashTagDetail) {
        ...
        if (strlen($request->search_names) >= 3) {
            ...
            if ($product_uses) {
                ...
                $product1s = \App\Product::where('products.go_live',1)->whereIn('product_code', $productuse_details->map(function ($productuse_detail) {
                    return $productuse_detail->Product_Code;
                }))->get();
                $products = $products->merge($product1s);
            }
