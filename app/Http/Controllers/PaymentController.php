...
class PaymentController extends Controller
{
    public function pay(Request $request)
    {
        ...
        $payment_request = \App\Payment::create([
            ...
            'ResponseTransID' => '',
            'Party_Code' => 0,
            'Requested_Amount' => number_format(round($grand_total_invoice), 2, '.', ''),
            'PaymentMode' => '',
        ...
        $payment->prepare([
            ...
            'callback_url' => $site_route . '/payment_callback', // callback URL
        ]);
    ...
    public function payment_callback(Request $request)
    {
        ...
        if ($transaction->isSuccessful()) {
            ...
            foreach ($product_carts as $product_cart) {
                ...
                if ($sales_scheme && $sales_scheme->SchemeOn_Code==1) {
                    $dividend = $product_cart->Qty;
                    $divisor = $sales_scheme->NextMinSaleQty_ForScheme;
                    $output = intdiv($dividend, $divisor);
            ...
            $exp_date = date('Y-m-d', strtotime($order->created_at->format('Y-m-d') . "+ 2 days"));
