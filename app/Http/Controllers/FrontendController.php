...
class FrontendController extends Controller
{
    ...
    public function prescription_print($id)
    {
        ...
        if ($data['groups']) {
            ...
            return view('frontend.prescription_print', $data);
        }
    }

    public function brand_page(Request $request, $request_group_name)
    {
        $brand = \App\Brand::where('url_name', '=', $request_group_name)->first();
        $groupcategories_list = \App\Groupcategory::where('group_id', $brand->id)->get();
        ...
        if (\Auth::user()) {
            if (\Auth::user()->role == 'Chemist') {
                if ($request->sort == 'ASC') {
                    $products = \App\Product::where('products.go_live',1)->whereIn('products.product_code', $product_group_categories->map(function ($product_group_category) {
                    ...
                    $sale_schemes = [];
                    if(\Auth::user()->role=='chemist'){
                        $sale_schemes = \App\SalesScheme::where('SchemeOn_Code',3)->where('schemefor',1)->get();
                    }else{
                        $sale_schemes = \App\SalesScheme::where('SchemeOn_Code',3)->where('SchemeOn_Code',2)->get();
                    }
                    
                    $products = \App\Product::where('products.go_live',1)->whereIn('products.product_code', $product_group_categories->map(function ($product_group_category) {
                        return $product_group_category->Product_Code;
                    }))->whereIn('product_code',$sale_schemes->map(function($sale_scheme){
                          return $sale_scheme->Product_Code;
                    }))->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                        ->where('productprice.ProductPriceType_Code', '=', '7')
                        ->select('products.*')
                        ->with('productprices')
                    ...
                    $products = \App\Product::where('products.go_live',1)->whereIn('product_code', $product_group_categories->map(function ($product_group_category) {
                        return $product_group_category->Product_Code;
                    }))->paginate(20);
                }
            } else {
        ...
        if ($brand) {
            if (\Auth::user()) {
                if (\Auth::user()->role == 'User') {
                    return view('frontend.users.brand_page', compact('uses', 'categories', 'main_sliders', 'brand', 'products', 'groups'));
                } elseif (\Auth::user()->role == 'Chemist') {

                    return view('frontend.chemists.brand_page', compact('uses', 'categories', 'main_sliders', 'products', 'brand', 'groups'));
                } else {
                    return view('frontend.brand_page', compact('uses', 'categories', 'main_sliders', 'brand', 'products', 'groups'));
                }
            } else {
                return view('frontend.brand_page', compact('uses', 'categories', 'main_sliders', 'brand', 'products', 'groups'));
            }
    ...
    public function group_page(Request $request, $request_group_name)
    {
        ...
        $uses = \App\Productuse::orderBy('ProductUse_Name','ASC')->get();
        if (\Auth::user()) {
            if (\Auth::user()->role == 'Chemist') {
                if ($request->sort == 'ASC') {
                    $products = \App\Product::where('products.go_live',1)->whereIn('products.product_code', $product_group_categories->map(function ($product_group_category) {
...
//   $data['PartyDetails']="{'Party_Code':1,'PartyType_Code':13,'Party_ID':101,'GPS':null,'Party_Name':'Test','Address1':'AddressLine1','Address2':'AddressLine2','Address3':'AddressLine3','City_Code':1,'City_Name':null,'State_Code':0,'State_Name':null,'PIN':null,'Phone':null,'DL_No':null,'PAN_No':null,'GSTIN':null,'Contact_Person':null,'Mobile_No':null,'Email':null,'Transporter_Code':0,'Transporter_Name':null,'Territory_Code':0,'Territory_Name':null,'TPArea_Code':0,'Area_Code':0,'Area_Name':null,'DistanceFromCity':0,'Photos':null,'LastVisit_Date':null,'LastInvoice_Date':null,'LastGoodsReceipt_Date':null,'LastInvoice_Value':0,'OutStanding_Value':null,'ApprovalStatus_Code':0,'Upload_Status':0,'CreatedBy':0,'Created_Date':null}";
    ...
    public function product_detail($group_url_name, $groupcategory_url_name, $brand_name)
    {
        ...
        $product = \App\Product::where('products.go_live',1)->where('url_name', '=', $brand_name)->first();
        ...
        if ($product->ProductBrand_Code == 1) {
            $product_group_categories = \App\ProductGroupCategories::where('Product_Code',$product->product_code)->get();
if(count($product_group_categories)){
    $all_product_list = \App\ProductGroupCategories::whereIn('groupcategory_id',$product_group_categories->map(function($product_group_category){
        return $product_group_category->groupcategory_id;
     }))->get();
     
         $products = \App\Product::where('products.go_live',1)->where('id', '!=', $product->id)->whereIn('products.product_code',$all_product_list->map(function($all_product){
             return $all_product->Product_Code;
         }))->get();
}else {
    $products = [];
}
            ...
            // $product_hashtags = \App\ProductHashTagDetail::where('Product_Code', $product->product_code)->get();
            // $product_code_arr = [];
            // foreach ($product_hashtags as $product_hashtag) {
            //     $product_hashtag_1 = \App\ProductHashTagDetail::where('ProductHashtag_Code', $product_hashtag->ProductHashtag_Code)->whereNotIn('Product_Code', [$product->product_code])->first();
            //     if ($product_hashtag_1) {
            //         $product_code_arr[] = $product_hashtag_1->Product_Code;
            //     }
            // }
            // if (count($product_code_arr)) {
            //     $products = \App\Product::where('products.go_live',1)->whereIn('product_code', $product_code_arr)->get();
            // } else {
            //     $products = [];
            // }
        }
    ...
    {
        $data['groups'] = \App\Group::with('groupcategories')->orderBy('id', 'DESC')->get();
        return view('frontend.users.book_an_appointment', $data);
    }

    public function book_an_appointment_store(Request $request)
    {
        $this->validate($request, [
            'user_id' => 'required',
        ...
        if (isset($request->upload_prescription)) {
            ...
            $doctor_appointment->file_attechment = $str_upload_prescription;
        ...
        $message = 'Thank you very much for uploading your prescription. The Doctor will call you shortly on your registered mobile number. Nestor Online "Life Comes First"';
        ...
        return redirect()->route('frontend.cart',['prescription_submit'=>'YES'])->with('success', 'Submitted Successfully! The Doctor will call you soon.');
    }
    ...
    public function checkout_upload_prescription(Request $request)
    {
        ...
        foreach ($add_to_cards as $add_to_card) {
            ...
            if ($product->Prescription_Required == 1) {
                if($add_to_card->doctor_description_id){

                }else{
                    return redirect()->route('frontend.book_an_appointment')->with('error',"Some selected medicines require Doctor's prescription. Please upload your prescription to avail Free Consultation with the Doctor to validate your prescription.");
                }

                // $doctor_appointment = \App\Doctorappointment::where('user_id', \Auth::user()->id)->orderBy('id', 'DESC')->first();
                // if ($doctor_appointment) {
                //     $doctor_prescribed_products = \App\DoctorPrescriptionProduct::where('product_id', $add_to_card->product_id)->where('doctorappointment_id', $doctor_appointment->id)->get();
                //     if (count($doctor_prescribed_products)) {
                //     } else {
        ...
        if (\Auth::user()) {
            ...
            if (\Auth::user()->role == 'User') {
                return redirect()->route('frontend.checkout',['reward'=>$request->reward]);
    ...
    public function checkout(Request $request)
    {
        ...
        foreach ($add_to_cards as $add_to_card) {
            $add_to_card_total = $add_to_card_total + $add_to_card->amount * $add_to_card->Qty;
        }
        if($request->reward){
            if($add_to_card_total<500){
                $reward = 0;
            }else{
        ...
        if (\Auth::user()->role == 'Chemist' && $add_to_card_total < $order_setting->MinimumOrderValueForChemist) {
            session()->flash('error', 'Minimum Order Value is Rs. ' . $order_setting->MinimumOrderValueForChemist . '/-');
            return redirect()->route('frontend.cart');
        } else if (\Auth::user()->role == 'User' && $add_to_card_total < $order_setting->MinimumOrderValueForCustomer) {
        ...
        $address = \App\Address::where('user_id', '=', \Auth::user()->id)->whereNotNull('Address1')->where('set_as_a_current', '=', 'Yes')->first();
        if (\Auth::user()) {
            if (\Auth::User()->role == 'Chemist' && $add_to_card_total < $order_setting->MinimumOrderValueForChemist) {
                session()->flash('error', 'Minmum Order Value is ' . $order_setting->MinimumOrderValueForChemist . '');
                return redirect()->route('frontend.cart');
            ...
            foreach ($add_to_cards as $add_to_card) {
                ...
                return view('frontend.users.checkout', compact('address', 'host', 'add_to_cards', 'wallet', 'groups', 'reward'));
            } elseif (\Auth::user()->role == 'Chemist') {
                return view('frontend.chemists.checkout', compact('address', 'host', 'add_to_cards', 'wallet', 'groups', 'reward'));
            } else {
                return view('frontend.checkout', compact('address', 'host', 'groups', 'reward'));
            }
        } else {
            return view('frontend.checkout', compact('address', 'host', 'groups', 'reward'));
        }
        return view('frontend.checkout', compact('address', 'host', 'add_to_cards', 'groups', 'wallet', 'reward'));
    }

    public function cart(Request $request)
    {
        $prescription_submit = $request->prescription_submit;
        ...
        $wallet = 0;
        if (\Auth::user()) {
            $reward_ledger = \App\RewardReferenceLedger::where('user_id', \Auth::user()->id)->orderBy('id', 'DESC')->first();
        ...
        if (\Auth::user()) {
            if (\Auth::user()->role == 'User') {
                return view('frontend.users.cart', compact('add_cart_datas', 'groups', 'wallet', 'order_setting','reward_ledger', 'site_route','prescription_submit'));
            } elseif (\Auth::user()->role == 'Chemist') {

                return view('frontend.chemists.cart', compact('add_cart_datas', 'groups', 'wallet', 'order_setting', 'reward_ledger', 'site_route'));
            } else {
                if ($add_cart_datas) {
                } else {
                    $add_cart_datas = [];
                }
                return view('frontend.cart', compact('add_cart_datas', 'groups', 'wallet', 'order_setting', 'site_route'));
            }
        } else {
            if ($add_cart_datas) {
    ...
    public function pincode_check(Request $request)
    {
        ...
        if (count($pincode)) {
            ...
            echo "Delivery by " . date("M d", strtotime("+2  day"));
        } else {
    ...
    public function order_tracking($order_id)
    {
        ...
        if ($order->Office_Code == 1) {
            $header = array('Content-Type: application/json', 'x-access-token: SL2409_trk:c8c183546b67b0db5f0ede565801f063');
        } else {
            $header = array('Content-Type: application/json', 'x-access-token: GL3870_trk:032caa809850a3b274b661a7a041d06b');
        }
        $tracking_data = [
            'trkType' => 'cnno',
            'strcnno' => "{$order->Tracking_ID}",
            'addtnlDtl' => 'Y',
        ];

//   $data['PartyDetails']="{'Party_Code':1,'PartyType_Code':13,'Party_ID':101,'GPS':null,'Party_Name':'Test','Address1':'AddressLine1','Address2':'AddressLine2','Address3':'AddressLine3','City_Code':1,'City_Name':null,'State_Code':0,'State_Name':null,'PIN':null,'Phone':null,'DL_No':null,'PAN_No':null,'GSTIN':null,'Contact_Person':null,'Mobile_No':null,'Email':null,'Transporter_Code':0,'Transporter_Name':null,'Territory_Code':0,'Territory_Name':null,'TPArea_Code':0,'Area_Code':0,'Area_Name':null,'DistanceFromCity':0,'Photos':null,'LastVisit_Date':null,'LastInvoice_Date':null,'LastGoodsReceipt_Date':null,'LastInvoice_Value':0,'OutStanding_Value':null,'ApprovalStatus_Code':0,'Upload_Status':0,'CreatedBy':0,'Created_Date':null}";
        $post_data = json_encode($tracking_data, JSON_UNESCAPED_SLASHES);
        ...
        $data['order'] = $order;
        $data['groups'] = \App\Group::with('groupcategories')->orderBy('id', 'DESC')->get();
        $data['trackings'] = json_decode($server_output);

        return view('frontend.order_tracking', $data);
    }

    public function show_pagination_filter_data(Request $request)
    {
       $input_parameter = $request->all();
        ...
        if ($request->groupcategory_id) {
            $product_group_categories = \App\ProductGroupCategories::where('groupcategory_id', $request->groupcategory_id)->get();
            ...
            $groupcategories_list = \App\Groupcategory::where('group_id', $request->group_id)->get();
            $product_group_categories = \App\ProductGroupCategories::whereIn('groupcategory_id', $groupcategories_list->map(function ($groupcategory) {
                return $groupcategory->id;
            }))->get();
            ...
            if (isset($request->category) && isset($request->prescription_required) && isset($request->uses)) {
                $productuse_details = \App\ProductuseDetail::whereIn('ProductUse_Code', $request->uses)->distinct()->get(['Product_Code']);
                $products = \App\Product::where('products.go_live',1)->whereIn('category_id', $request->category)
                    ->whereIn('Prescription_Required', $request->prescription_required)
                    ->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereIn('productprice.Product_Code', $productuse_details->map(function ($productuse_detail) {
                ...
                $products = \App\Product::where('products.go_live',1)->whereIn('ProductUse_Code', $request->uses)
                    ->whereIn('Prescription_Required', $request->prescription_required)->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereIn('productprice.Product_Code', $productuse_details->map(function ($productuse_detail) {
                        return $productuse_detail->Product_Code;
                ...
                $products = \App\Product::where('products.go_live',1)->whereIn('Prescription_Required', $request->prescription_required)
                    ->orderBy('id', 'DESC')->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereBetween('productprice.Price', array($request->minval, $request->maxval))
                    ->select('products.*')->paginate(20);
                ...
                $productuse_details = \App\ProductuseDetail::whereIn('ProductUse_Code', $request->uses)->distinct()->get(['Product_Code']);
                $products = \App\Product::where('products.go_live',1)->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereIn('productprice.Product_Code', $productuse_details->map(function ($productuse_detail) {
                        return $productuse_detail->Product_Code;
                    }))->whereBetween('productprice.Price', array($request->minval, $request->maxval))
        ...
        if (\Auth::user()) {
            if (\Auth::user()->role == 'Chemist') {
                return view('frontend.chemists.show_pagination_filter_data', compact('products', 'groups', 'single_group', 'single_groupcategory', 'categories', 'uses', 'main_sliders','input_parameter'));
            } elseif (\Auth::user()->role == 'User') {
                return view('frontend.users.show_pagination_filter_data', compact('products', 'groups', 'single_group', 'single_groupcategory', 'categories', 'uses', 'main_sliders','input_parameter'));
            } else {
                return view('frontend.show_pagination_filter_data', compact('products', 'groups', 'single_group', 'single_groupcategory', 'categories', 'uses', 'main_sliders','input_parameter'));
            }
        } else {
            return view('frontend.show_pagination_filter_data', compact('products', 'groups', 'single_group', 'single_groupcategory', 'categories', 'uses', 'main_sliders','input_parameter'));
        }
    ...
    public function sidebar_filter_data(Request $request)
    {
        ...
        if ($request->groupcategory_id) {
            ...
            if (isset($request->category) && isset($request->prescription_required) && isset($request->uses)) {
                ...
                $products = \App\Product::where('products.go_live',1)->whereIn('products.product_code', $product_group_categories->map(function ($product_group_category) {
                    return $product_group_category->Product_Code;
                }))->whereIn('products.category_id', $request->category)
                    ->whereIn('products.Prescription_Required', $request->prescription_required)
                    ->orderBy('id', 'DESC')->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereIn('productprice.Product_Code', $productuse_details->map(function ($productuse_detail) {
                        return $productuse_detail->Product_Code;
                ...
                $products = \App\Product::where('products.go_live',1)->whereIn('products.product_code', $product_group_categories->map(function ($product_group_category) {
                    return $product_group_category->Product_Code;
                }))->whereIn('products.Prescription_Required', $request->prescription_required)->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereIn('productprice.Product_Code', $productuse_details->map(function ($productuse_detail) {
                        return $productuse_detail->Product_Code;
                    }))->whereBetween('productprice.Price', array($request->minval, $request->maxval))
            ...
            if (isset($request->category) && isset($request->prescription_required) && isset($request->uses)) {
                ...
                $products = \App\Product::where('products.go_live',1)
                    ->whereIn('Prescription_Required', $request->prescription_required)->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereIn('productprice.Product_Code', $productuse_details->map(function ($productuse_detail) {
                        return $productuse_detail->Product_Code;
                ...
                $products = \App\Product::where('products.go_live',1)->whereIn('category_id', $request->category)
                    ->whereIn('products.ProductUse_Code', isset($request->uses))
                    ->orderBy('id', 'DESC')->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereIn('productprice.Product_Code', $productuse_details->map(function ($productuse_detail) {
                        return $productuse_detail->Product_Code;
