...
class APIController extends Controller
{
    ...
    public function search_App(Request $request)
    {
        ...
        if ($request->name) {
            $product_list = [];
            $products = \App\Product::where('brand_name', 'LIKE', $request->name . '%')->orWhere('generic_name', 'LIKE', $request->name . '%')->limit(5)->get();
            if (strlen($request->search_names) >= 1) {
                $product1s = \App\Product::where('brand_name', 'LIKE', '%' . $request->name . '%')->where('ProductBrand_Code', 2)->limit(5)->get();
                if ($product1s) {
                    $products = $products->merge($product1s);
                }
                $product2s = \App\Product::where('brand_name', 'LIKE', '%' . $request->name . '%')->where('ProductBrand_Code', 3)->limit(5)->get();
                if ($product1s) {
            ...
            if (strlen($request->name) >= 1) {
                ...
                if (count($ProductHashTags)) {
                    ...
                    $product1s = \App\Product::whereIn('product_code', $ProductHashTagDetails->map(function ($ProductHashTagDetail) {
    ...
    public function product_detail_App(Request $request)
    {
        ...
        if ($single_product) {
            ...
            $product_price = \App\Productprice::where('Product_Code', '=', $single_product->product_code)->where('ProductPriceType_Code', '=', '9')->first();
    ...
    public function chemist_home_App(Request $request)
    {
        ...
        if ($user) {
            ...
            $products = \App\Product::select(['id', 'generic_name', 'brand_name', 'image', 'offer', 'product_code', 'package_id'])->skip(1)->limit('20')->get();
            foreach ($products as $product) {
...
//         $a1['order_history'] = $order_history;
    ...
    public function our_products_App(Request $request)
    {
        ...
        $site_route = $request->getSchemeAndHttpHost();
        $products = \App\Product::select(['id', 'generic_name', 'brand_name', 'image', 'offer', 'product_code', 'group_id', 'groupcategory_id', 'package_id'])->skip(1)->limit(500)->orderBy('id', 'ASC')->paginate(20);
        if (count($products)) {
            foreach ($products as $product) {
