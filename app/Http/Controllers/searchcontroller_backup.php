...
class SearchController extends Controller
{
    ...
    public function get_search_data_after_filter_App(Request $request)
    {
        ...
        if (isset($request->groupcategory_id)) {
            ...
            if (isset($request->category) && isset($request->prescription_required) && isset($request->uses)) {
                ...
                $products = \App\Product::whereIn('groupcategory_id', $request->groupcategory_id)->whereIn('category_id', $request->category)
                ...
                $products = \App\Product::whereIn('groupcategory_id', $request->groupcategory_id)->whereIn('ProductUse_Code', $request->uses)
                ...
                $products = \App\Product::where('ProductBrand_Code', '=', $request->ProductBrand_Code)->whereIn('category_id', $request->category)
                ...
                $products = \App\Product::whereIn('groupcategory_id', $request->groupcategory_id)->whereIn('Prescription_Required', $request->prescription_required)
                    ->orderBy('id', 'DESC')->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereBetween('productprice.Price', array($request->minval, $request->maxval))
                    ->select('products.*')->paginate(20);
                ...
                $productuse_details = \App\ProductuseDetail::where('ProductUse_Code', $request->uses)->distinct()->get(['Product_Code']);
                $products = \App\Product::whereIn('groupcategory_id', $request->groupcategory_id)->join('productprices as productprice', 'productprice.Product_Code', '=', 'products.product_code')
                    ->where('productprice.ProductPriceType_Code', '=', $price_code)
                    ->whereIn('productprice.Product_Code', $productuse_details->map(function ($productuse_detail) {
                        return $productuse_detail->Product_Code;
                    }))->whereBetween('productprice.Price', array($request->minval, $request->maxval))
    ...
    {
        $products = \App\Product::where('brand_name', 'LIKE', '%' . $request->input('search_names') . '%')->orWhere('generic_name', 'LIKE', '%' . $request->input('search_names') . '%')->get();
