...
class ChemistsExport implements FromCollection, WithHeadings, ShouldAutoSize, WithColumnWidths, WithStyles
{
    /**
    ...
    public function collection()
    {
        ...
        if (request()->input('search_from_date') && request()->input('search_to_date')) {
            $users = \App\User::whereBetween('created_at', [request()->input('search_from_date') . ' 00:00:00', request()->input('search_to_date') . ' 23:59:59'])->where('role', 'Chemist')->where('status', 'verify')->get();
        } else {
            $users = \App\User::where('role', 'Chemist')->where('status', 'verify')->get();
        }

        $output = [];
        foreach ($users as $key => $user) {

            if (request()->input('State_Code')) {
                $chemist = \App\Chemist::where('user_id', $user->id)->where('State_Code', request()->input('State_Code'))->where('Status', 1)->first();
            } else {
                $chemist = \App\Chemist::where('user_id', $user->id)->where('State_Code', request()->input('State_Code'))->where('Status', 1)->first();
            }
